...
object MetalDetectorSolver {
    ...
    @Suppress("unused")
    private val tickEvent =
        listenEvent<TickEvent, Unit> {
            if (!divanCheck()) return@listenEvent
            if (successWaypoint != null && minecraft.player?.position()?.distanceTo(successWaypoint!!.pos)!! < 1.5) successWaypoint = null
            if (minesCenter != null) return@listenEvent
    ...
    private fun solve(
        ...
        val distances =
            actualChestPositions?.associateWith {

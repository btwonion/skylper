...
object PlayerChatLocationListener {
    ...
    private val messageEvent =
        listenEvent<MessageEvent, Unit> { event ->
            ...
            val loc =
                regex.find(rawMessage)?.groupValues?.toMutableList()?.also { values ->
    ...
    private fun handleRawLocation(
        location: RawLocation,
        rawMessage: String
    ) {
        if (config.mining.crystalHollows.automaticallyAddLocations) {
            val matchingSpecific =
                PreDefinedHollowsLocationSpecific.entries.find {
                    rawMessage.contains(it.name, true) ||
                        rawMessage.contains(
                            it.key,
                            true
                        )
        ...
        PreDefinedHollowsLocationSpecific.entries.forEach { specific ->
            minecraft.player?.sendSystemMessage(
                Component.translatable(
                    ...
                    specific.displayName.copy().withStyle(ChatFormatting.RED)
                ).withStyle {
                    val command =

...
object PowderGrindingTracker : Tracker<PowderGrindingData>("hollows.powder_grinding", PowderGrindingData()) {
    ...
    @Suppress("unused")
    private val bossBarListener =
        listenEvent<BossBarNameUpdate, Unit> { (text) ->
            if (!HollowsModule.isPlayerInHollows) return@listenEvent
            val raw = text.string
    ...
    override fun createComponents(data: PowderGrindingData): List<Component> {
        fun finalComponent(
            ...
            val name = Component.translatable("$overlayNameSpace.$key").withStyle { it.withBold(true).withColor(ChatFormatting.AQUA) }
        ...
        return buildList {
            ...
            data.gemstone.getComponent(config.gemstone, this@PowderGrindingTracker)?.let { add(finalComponent("gemstone", it)) }
            data.mithril.getComponent(config.mithril, this@PowderGrindingTracker)?.let { add(finalComponent("mithril", it)) }
    ...
    override val updateTriggerEvents: List<KClass<out Event<out Any>>> = listOf(BossBarNameUpdate::class, MessageEvent::class)
